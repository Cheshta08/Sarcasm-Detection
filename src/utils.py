...
def load_model(json_name, h5_weights_name, verbose=False):
    ...
    model = model_from_json(loaded_model_json, custom_objects={'f1_score': f1_score})
...
def load_vectors(filename='glove.6B.100d.txt'):
    ...
    word2vec_map = {}
    # Adjust path based on the filename
    if 'emoji' in filename:
        file_path = path + '/models/emoji2vec/' + filename
    else:
        file_path = path + '\\res\\glove\\' + filename
    ...
    print('Found %s word vectors with embedding dimension %s'
...
def convert_emoji_to_unicode(emoji):
    unicode_emoji = emoji.encode('unicode-escape').decode('ascii')
    print(f"Original emoji: {emoji}, Unicode emoji: {unicode_emoji}")
    unicode_emoji = unicode_emoji.replace('\\ufe0f', '')  # Remove VS15
    unicode_emoji = unicode_emoji.replace('\\ufe0e', '')  # Remove VS16
...
def make_analogy(a, b, c, vec_map):
    ...
    # print(f"Converted emojis: {a}, {b}, {c}")

    if a not in vec_map:
        print(f"Key {a} not found in vec_map")
    if b not in vec_map:
        print(f"Key {b} not found in vec_map")
    if c not in vec_map:
        print(f"Key {c} not found in vec_map")
    ...
    print("Top 5 most similar emojis: ", [r for r in sorted_keys[:5]])
...
def plot_training_statistics(history, plot_name, also_plot_validation=False, acc_mode='accuracy', loss_mode='loss'):
    # Plot Accuracy
...
def initialize_writer(to_write_filename):
    fout = open(to_write_filename, 'wt', encoding='utf-8')
