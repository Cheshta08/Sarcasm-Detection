...
def compute_bert_embeddings(data, batch_size=32, emb_file_path=None):
    """Compute BERT embeddings in batches and save/load embeddings."""
    if emb_file_path and os.path.exists(emb_file_path):
        # If embeddings file exists, load and return
    ...
    for batch in data_batches:
        ...
        embeddings.append(bert_encoder(preprocessed)['sequence_output'])
    
    embeddings = tf.concat(embeddings, axis=0).numpy()
    
    # Save embeddings to a file if the file path is provided
    if emb_file_path:
        print(f"Saving embeddings to {emb_file_path}...")
...
def nn_bow_model(x_train, y_train, x_test, y_test, results, mode,
                 epochs=5, batch_size=32, hidden_units=5, save=False, plot_graph=False):
...
def standard_dnn_model(input_layer, hidden_units, dropout):
    # Standard DNN architecture with a Flatten layer
    X = Dense(hidden_units, kernel_initializer='he_normal', activation='relu')(input_layer)
...
#         assert len(input_shape) == 3
#         self.a = self.add_weight(shape=(input_shape[-1],), initializer=self.init, name='lstm_weight', trainable=True)
#         super(MyAttentionLayer, self).build(input_shape)
...
#     return attention
def cnn_model(input_layer, hidden_units,dropout):
    X = Conv1D(filters=hidden_units, kernel_size=3, kernel_initializer='he_normal', padding='valid', activation='relu')(change_layer)
    X = Conv1D(filters=hidden_units, kernel_size=3, kernel_initializer='he_normal', padding='valid', activation='relu')(X)
    X = GlobalMaxPooling1D()(X)
    return X


# LSTM Model
def lstm_model(input_layer, hidden_units, dropout):
    X = LSTM(hidden_units, kernel_initializer='he_normal', activation='tanh', dropout=dropout, return_sequences=True)(change_layer)
...
# GRU Model
def gru_model(input_layer, hidden_units, dropout):
    X = GRU(hidden_units, kernel_initializer='he_normal', activation='tanh', dropout=dropout, return_sequences=True)(input_layer)
    X = GRU(hidden_units, kernel_initializer='he_normal', activation='tanh', dropout=dropout, return_sequences=False)(X)
    return X


# Bidirectional LSTM Model
def bidirectional_lstm_model(input_layer, hidden_units, dropout):
    X = LSTM(hidden_units, kernel_initializer='he_normal', activation='tanh', dropout=dropout, return_sequences=True)(input_layer)
    X = Bidirectional(LSTM(hidden_units, kernel_initializer='he_normal', activation='sigmoid', dropout=dropout, return_sequences=False))(X)
    return X


# CNN + LSTM Model
def cnn_lstm_model(input_layer, hidden_units, dropout):
    X = Conv1D(hidden_units, 3, kernel_initializer='he_normal', padding='valid', activation='relu')(input_layer)
...
# Stateless Attention Model
def stateless_attention_model(input_layer, hidden_units, dropout, max_tweet_length=50):
    X = LSTM(hidden_units, kernel_initializer='he_normal', activation='tanh', dropout=dropout, return_sequences=True)(input_layer)
    attention_layer = Permute((2, 1))(X)
    attention_layer = Dense(max_tweet_length, activation='softmax')(attention_layer)
    attention_layer = Lambda(lambda x: K.mean(x, axis=1), name='dim_reduction')(attention_layer)
    attention_layer = RepeatVector(int(X.shape[2]))(attention_layer)
    attention_probabilities = Permute((2, 1), name='attention_probs')(attention_layer)
    attention_layer = Multiply()([X, attention_probabilities])
...
def attention_model(input_layer, hidden_units, dropout):
    lstm_out = LSTM(hidden_units, kernel_initializer='he_normal', activation='tanh', dropout=dropout, return_sequences=True)(input_layer)
    attention = MyAttentionLayer()(lstm_out)
    return attention
...
def build_model(input_shape, hidden_units, dropout, dnn_architecture):
    ...
    # Pass the embeddings through the DNN architecture
    X = dnn_architecture( input_layer=input_layer, 
...
#     ratio = utils.get_classes_ratio_as_dict(train_labels)
...
def run_dl_analysis(train_tweets, test_tweets, y_train, y_test, path, shuffle=False,
    ...
    train_embeddings_file = path + '\\models\\bert_embeddings_train-1.npy'
    test_embeddings_file = path + '\\models\\bert_embeddings_test-1.npy'
    
    train_embeddings = compute_bert_embeddings(train_tweets, batch_size, emb_file_path=train_embeddings_file)
    test_embeddings = compute_bert_embeddings(test_tweets, batch_size, emb_file_path=test_embeddings_file)
    ...
    for dnn_model in dnn_models:
        ...
        # Build the deep neural network architecture
        utils.print_model_title(dnn_model)
        model = build_model(train_embeddings.shape[1], hidden_units, dropout, dnn_architecture=dnn_options(dnn_model))
       ...
       filepath=os.path.join(path, 'models', 'dnn_models', 'best', dnn_model.lower() + '_model.keras')  # Save as .h5
        ...
        # Fit the model on the training data
        history = model.fit(train_embeddings, y_train, batch_size=batch_size, epochs=epochs, shuffle=True, class_weight=ratio,
                            callbacks=[save_best, reduceLR, early_stopping], validation_split=0.1, verbose=1)
        ...
        # Make prediction and evaluation
        predict(model, test_embeddings, y_test)
        end = time.time()
        print("==================================================================\n")
        print("%s model analysis completion time: %.3f s = %.3f min"
              % (dnn_model, (end - start), (end - start) / 60.0))
...
if __name__ == "__main__":
    ...
    to_write_filename = path + '/stats/dnn_models_analysis-1.txt'
    ...
    for emb_type in emb_types:
        ...
        models = ['CNN','LSTM', 'GRU', 'Bidirectional LSTM', 'CNN + LSTM', 'Stateless Attention', 'Stateless Attention']
        # 'CNN','LSTM', 'GRU', 'Bidirectional LSTM', 'CNN + LSTM', 'Stateless Attention', 'Stateless Attention',
