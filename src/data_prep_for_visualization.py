...
def prepare_data(shuffle=False, labels_to_categorical=True):
    ...
    data_path = path + "\\res\\tokens\\tokens_"
    ...
    # Load the labels
    train_labels = [int(l.strip()) for l in utils.load_file(path + "\\res\\datasets\\labels_" + train_filename)]
    test_labels = [int(l.strip()) for l in utils.load_file(path + "\\res\\datasets\\labels_" + test_filename)]
